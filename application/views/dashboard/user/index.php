...
$barang = array_unique(array_column($this->m_user->getbarang(), 'nama_barang'));
?>
...
<div class="container-fluid">
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title"><i class="mdi mdi-calendar"></i> Ketik Tahun Untuk Input Aset Desa</h5>
                </div>
                <div class="card-body">
                    <div class="form-group">
                        <label for="tahun">Tahun</label>
                        <input type="text" class="form-control col-lg-3" name="tahun" id="tahun" placeholder="<?= date('Y') ?>">
                    </div>
                </div>
                <div class="card-footer d-flex align-items-center justify-content-end">
                    <button type="button" class="btn btn-success" id="show_assets" onclick="return window.location = '<?= base_url('index.php/user/assets/view') ?>';"><i class="mdi mdi-checkbox-marked-outline"></i> OK</button>
                </div>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title">Daftar Aset Desa Tahun - <?= date('Y') ?></h5>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-responsive-xs table-responsive-sm" id="datatable">
                            ...
                            <tbody>
                                ...
                                $total_penyusutan_gedung    = 0;
                                $total_penyusutan_jalan     = 0;
                                ...
                                foreach($inventaris as $data):
                                    if($data->kategori_inventaris === 'tnh-desa') {
                                        ...
                                        $total_nilai_buku[$data->kode_barang][] = $nilai_buku; 

                                    }

                                    if(preg_match('/^3/', $data->kode_barang)) {
                                        $total_penyusutan_peralatan += $penyusutan;
                                    } elseif(preg_match('/^4/', $data->kode_barang)) {
                                        $total_penyusutan_gedung += $penyusutan;
                                    } elseif(preg_match('/^5/', $data->kode_barang)) {
                                        $total_penyusutan_jalan += $penyusutan;
                                    }
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            ...
                            foreach($barang as $data): 
                                $kode_barang = $this->db->get_where('tb_barang', ['nama_barang' => $data])->row()->kode_barang;

                                if(@$total_nilai_buku) {
                                    if(array_key_exists($kode_barang, $total_nilai_buku)) {
                                        $sum[$kode_barang] = 0;
                                        foreach($total_nilai_buku[$kode_barang] as $total) {
                                            $sum[$kode_barang] += $total;
                                        }
                                    }
                                }
                            ?>
                            <div class="form-group">
                                <label for="alat"><?= $data ?></label>
                                <input type="text" class="form-control col-lg-8" value="<?= number_format(round(@$sum[$kode_barang]), 0, '', '.') ?>">
                            </div>
                            <?php endforeach; ?>
                        </div>
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label for="peralatan">Akumulasi Total Penyusutan Peralatan dan Mesin</label>
                                <input type="text" class="form-control col-lg-8" value="<?= number_format(round($total_penyusutan_peralatan), 0, '', '.') ?>">
                            </div>
                            <div class="form-group">
                                <label for="gedung">Akumulasi Total Penyusutan Gedung dan Bangunan</label>
                                <input type="text" class="form-control col-lg-8" value="<?= number_format(round($total_penyusutan_gedung), 0, '', '.') ?>">
                            </div>
                            <div class="form-group">
                                <label for="jalan">Akumulasi Total Penyusutan Jalan, Irigasi dan Jaringan</label>
                                <input type="text" class="form-control col-lg-8" value="<?= number_format(round($total_penyusutan_jalan), 0, '', '.') ?>">
                            </div>
                            <div class="form-group">
                                <label for="KDP">Akumulasi Total Penyusutan</label>
                                <input type="text" class="form-control col-lg-8" value="<?= number_format(round($total_penyusutan), 0, '', '.') ?>">
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    function doEdit(id) {
        $.post('<?= base_url('index.php/user/doGetInventaris') ?>', {id: id}, (data) => {
            ...
            $('#editInventarisModal form #tgl_beli').val(tgl + '-' + bulan + '-' + tahun);
    ...
    $(function() {
        ...
        $(document).on('keyup', '.form-group > #tahun', function() {
            let tahun = parseInt($(this).val());

            $('#show_assets').attr('onclick', 'return window.location = "<?= base_url('index.php/user/assets/view') ?>/' + tahun + '"');
        });
